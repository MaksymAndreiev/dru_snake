...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            available_positions = list(zip(*np.where(self.world == 0)))
            l = len(available_positions)
            start_position = available_positions[np.random.randint(0, l)]
            # Choose a random direction index
            start_direction_index = np.random.randint(0, len(self.DIRECTIONS))
            new_snake = Snake(start_position, start_direction_index, SNAKE_SIZE)
            c = 0
            while c != 3:
                for block in new_snake.blocks:
                    if block not in available_positions:
                        start_direction_index = np.random.randint(0, len(self.DIRECTIONS))
                        new_snake = Snake(start_position, start_direction_index, SNAKE_SIZE)
                    else:
                        c += 1
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = self.available_food_positions.copy()
        copy = self.current_available_food_positions.copy()
        try:
            for pos in copy:
                if pos in snake.blocks:
                    self.current_available_food_positions.remove(pos)
        except ValueError:
            pass
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.world[self.snake.blocks[0][0], self.snake.blocks[0][1]] == 255:
            ...
            if new_snake_head == self.food_position:
                ...
                tail = tuple(old_snake_tail - self.DIRECTIONS[action])
                self.snake.blocks.append(tail)
